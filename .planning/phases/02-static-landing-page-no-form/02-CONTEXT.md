...
### Decision Log Update (post-review, 2026-04-27)
The cross-AI review (`02-REVIEWS.md`) flagged the placeholder hero/secondary CTA's `<a href="#waitlist">` self-anchor as a no-op UX (clicking scrolls to the section the user is already in). The review also flagged HIGH-severity LCP risk (sub-headline `<p>` may win LCP over H1 due to larger painted area on 320px viewports), HIGH-severity above-fold fitting risk (vertical stack too tall on 320×568), MEDIUM footer tap-target risk, and HIGH CI-gate risk (workflow runs but does not block merges without branch protection). These resolve via the new D-30..D-34 decisions below plus targeted edits to plans 02-01..02-05 and a new regression-test plan 02-06.

- **D-30:** Above-fold spacing tightening for 320×568 (HIGH from review). Hero `<section>` padding changes from `py-16 md:py-24` (LOCKED in original D-17 for hero only) to `py-8 md:py-16 lg:py-24`. The hero inner flex container's vertical gap changes from `gap-6` to `gap-4`. The microcopy spacing under the CTA changes from `mt-3` to remain `mt-3` (the `mt-6` between sub-headline and CTA wrapper changes to `mt-4`). Sub-headline width tightens from `max-w-prose` to `max-w-xs sm:max-w-prose` so the wrapped H1 has demonstrably more painted area than the wrapped sub-headline at 320px (LCP-area defense). D-17's `py-16 md:py-24` rhythm REMAINS for `<PlaceholderFormSection>`, `<WhyQuibly>`, `<FounderVoice>`, and `<SecondaryCTA>` — only the hero relaxes. Reason: re-derived 320×568 vertical math (review §Codex Plan 02-02) shows the prior hero spacing left no margin after browser chrome.

- **D-31:** Placeholder hero/secondary CTA pills render as `<button type="button" aria-disabled="true">` — NOT as `<a href="#waitlist">`. Copy changes from "Join the waitlist" → "Form coming soon" on the hero CTA only; the secondary CTA copy stays "Don't miss launch — join the waitlist" but its element type also flips to `<button type="button" aria-disabled="true">`. Both buttons render visually identical to the locked Phase 2 styling but cannot be clicked to a no-op self-anchor. Phase 3 replaces both `<button aria-disabled>` instances with the real form's submit `<button>` (hero CTA) and a real `<a href="#waitlist">` smooth-scroll back-pointer (secondary CTA, when the form is several thousand pixels above the bottom). The `id="waitlist"` anchor target on `<PlaceholderFormSection>` REMAINS in Phase 2 — Phase 3 still needs the seam — it just isn't pointed at by any Phase-2 control. Reason: avoids no-op self-anchor UX flagged by Codex (review §Plan 02-02 and Consensus); honors WCAG 2.1 SC 4.1.2 (Name, Role, Value) by giving disabled controls explicit `aria-disabled="true"` rather than visually-misleading active anchors.

- **D-32:** Footer tap targets bumped to `min-h-12 inline-flex items-center` (≥48px effective height, MOB-02 compliance) on each `<a>` in `components/sections/footer.tsx`. The `text-sm` font size REMAINS (footer typography per UI-SPEC); only the link container's min-height grows. Reason: footer link `text-sm` (14px) + `py-2` (16px combined vertical padding) yielded an effective 30–46px tap target, below MOB-02's 48px floor. `min-h-12` (48px Tailwind unit) guarantees ≥48px regardless of font line-height. Codex flagged HIGH; Gemini did not flag.

- **D-33:** Lighthouse CI assertion `render-blocking-resources` promoted from `["warn", { "maxLength": 0 }]` to `["error", { "maxLength": 0 }]`. Phase 2 SC #4 wording is unconditional ("no render-blocking third-party scripts on first paint") — `warn` does not enforce. Phase 2 currently ships zero render-blocking third-party JS, so the gate flips green on its own. If a future plan introduces such a script, the gate fires HARD instead of merely warning. Reason: Codex MEDIUM; aligns gate with the success-criterion wording.

- **D-34:** Branch protection on `main` requires the `Lighthouse CI / lighthouse` status check (the GitHub-rendered name of the workflow's job). This is a one-time manual GitHub UI step (Settings → Branches → Branch protection rules → main → Require status checks → search "Lighthouse CI / lighthouse" → Add → Save). Plan 02-05's checkpoint task confirms this is set; CI gate is otherwise advisory only. Reason: Codex HIGH (review §Plan 02-05); without branch protection, the workflow runs but cannot block merge.
...
*Decision Log Update applied: 2026-04-27 (post-review — D-30..D-34 added)*
